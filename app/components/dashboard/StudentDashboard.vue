    ...
    <template v-else>
      ...
      >
        <div class="text-center">
          ...
          >
            Your account is not currently connected to a student record.
            ...
            <div class="flex items-center gap-3">
              ...
              <div>
                ...
                <p class="mt-0.5 text-sm text-gray-500 dark:text-gray-400">
                  Track your submitted and remaining evaluations.
                </p>
              </div>
            </div>
...
/*
|--------------------------------------------------------------------------
| Temporary expected evaluation count
|--------------------------------------------------------------------------
|
| Replace this value later with the actual number of assigned evaluations.
| Your original component also used a fixed total of five.
|
*/

const expectedTotalEvaluations = ref(0)
...
const pending = computed(() =>
  Math.max(
    0,
    expectedTotalEvaluations.value - submitted.value
  )
)
...
const completionPercentage = computed(() => {
  ...
  const percentage =
    (submitted.value / expectedTotalEvaluations.value) * 100
...
const studentStats = computed(() => [
  ...
  },
  {
    ...
    description: 'Evaluations waiting for completion',
  ...
  },
  {
    ...
    description: 'Overall evaluation completion',
  ...
  },
  {
    ...
    value: expectedTotalEvaluations.value.toLocaleString(),
    description: 'Total expected evaluation assignments',
    ...
    badge: 'Assigned',
    suffix: '',
    footerLabel: 'Evaluation requirement',
    footerValue: 'Current Period',
...
const getStudentRecord = async () => {
  ...
  /*
   * Expected Strapi relation:
   *
   * Student.user -> Users & Permissions User
   */
...
const getEvaluations = async () => {
  ...
  const response = await $api('/evaluations', {
    query: {
      /*
       * Your original code used filters[user][id].
       *
       * Change "user" to "evaluator_user" when your Evaluation
       * content type stores the student account using the
       * evaluator_user relation.
       */
