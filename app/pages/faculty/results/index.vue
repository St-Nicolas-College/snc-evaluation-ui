      ...
      >
        <div class="flex min-w-0 items-start gap-4">
          ...
          <div class="min-w-0">
            ...
            >
              Review your evaluation performance, ratings, feedback,
              and academic-period results from students and administrators.
            </p>
          </div>
        </div>

        <div
          class="grid grid-cols-3 gap-2 sm:min-w-[390px]"
        >
          <div
        ...
        >
          <div class="flex items-center justify-between">
            <div>
              ...
              <UBadge
                :color="getScoreColour(Number(overallAverage))"
                variant="subtle"
                size="sm"
                class="mt-1.5"
              >
        ...
        >
          <div class="flex items-start gap-3">
            ...
            <div>
              ...
              >
                Overall interpretation based on available evaluation records.
              </p>
            </div>
          </div>
          ...
          <div
            ...
            :class="getScoreContainerClass(Number(overallAverage))"
          >
            <p
              class="text-xs font-medium text-gray-500 dark:text-gray-400"
            >
        ...
        >
          ...
          <div class="space-y-4 p-4">
            ...
            <div
              ...
              :class="getScoreContainerClass(getAverageScore(result))"
            >
              <div
                class="flex items-center justify-between gap-3"
              >
                <div>
                  ...
                  <div
                    class="mt-1 flex items-end gap-1.5"
                  >
                    <p
                  ...
                  <UBadge
                    :color="getScoreColour(getAverageScore(result))"
                    variant="subtle"
                    size="sm"
                    class="mt-1.5"
                  >
                    {{ getScoreLabel(getAverageScore(result)) }}
                  </UBadge>
                </div>
                ...
                <UIcon
                  name="i-lucide-chart-no-axes-combined"
                  class="size-7"
                  :class="getScoreIconClass(getAverageScore(result))"
                />
              </div>
            </div>
            ...
            >
              ...
              >
                ...
                >
                  {{
                    formatDate(
                      getEvaluationDate(selectedResult)
                    )
                  }}
                </p>
              </div>
              ...
              >
                ...
                >
                  {{
                    formatScore(
                      getAverageScore(selectedResult)
                    )
                  }}/5
                </p>
              </div>
...
const selectedSourceLabel = computed(() => {
  return (
    sourceFilterOptions.find(
      item =>
        item.value === selectedSource.value
    )?.label ||
    'All Sources'
  )
})
...
const studentEvaluationCount = computed(() => {
  return studentResults.value.length
})

const deanEvaluationCount = computed(() => {
  return deanResults.value.length
})
...
const calculateCollectionAverage = (
  collection: any[]
) => {
  const scores = collection
    .map(result =>
      getNormalisedAverageScore(result)
    )
...
const studentAverage = computed(() => {
  return calculateCollectionAverage(
    studentResults.value
  )
})
...
const getNormalisedAverageScore = (
  result: any
) => {
  const score =
    getRawAverageScore(result)
  ...
  /*
    Dean–Faculty forms may use a 4-point scale.
    Convert a 4-point average to the 5-point display
    used by the unified results dashboard.
  */
  if (
    getSourceType(result) === 'dean' &&
    score <= 4
...
const getAverageScore = (
  ...
  return getNormalisedAverageScore(
    result
  )
}
...
const getFacultyResults = async () => {
  ...
  const teacherId =
    facultyProfile.value.id

  const response = await $api(
    '/evaluations',
    {
      query: {
        'filters[teacher][id][$eq]':
          teacherId,
