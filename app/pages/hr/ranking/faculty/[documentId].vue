<template>
  <div class="space-y-6 pb-10">
    ...
    <div class="flex flex-wrap items-center justify-between gap-3">
      ...
      <div class="flex flex-wrap gap-2">
        ...
        <UButton
          ...
          :disabled="loading || computing || isGeneratingPdf"
    ...
    <div v-if="loading" class="space-y-4">
      <USkeleton class="h-52 rounded-[28px]" />
      <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <USkeleton v-for="i in 4" :key="i" class="h-32 rounded-2xl" />
      </div>
      <USkeleton class="h-[480px] rounded-2xl" />
    </div>

    <!-- ERROR -->
    <section
      v-else-if="loadError"
      class="rounded-[28px] border border-red-200 bg-red-50 p-10 text-center dark:border-red-900 dark:bg-red-950/20"
    >
      <div
    ...
    <template v-else-if="teacher">
      <!-- HERO -->
      <section
        class="relative overflow-hidden rounded-[32px] border border-white/10 bg-gradient-to-br from-slate-950 via-blue-950 to-cyan-900 px-6 py-7 text-white shadow-xl sm:px-8 sm:py-8"
      >
        <div
          class="pointer-events-none absolute -right-16 -top-24 size-72 rounded-full bg-cyan-400/10 blur-3xl"
        />
        <div
          class="pointer-events-none absolute -bottom-24 left-1/3 size-72 rounded-full bg-blue-400/10 blur-3xl"
        />
        ...
        >
          <div class="flex min-w-0 items-center gap-4">
            ...
            <div class="min-w-0">
              <div class="flex flex-wrap items-center gap-2">
                <span
                  class="rounded-full border border-cyan-300/20 bg-cyan-300/10 px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.16em] text-cyan-200"
                >
                  Ranking Breakdown
                </span>

                <span
                  class="rounded-full border border-white/15 bg-white/10 px-2.5 py-1 text-[10px] font-semibold text-blue-100"
                >
                  {{ teacherRole }}
                </span>
              </div>

              <h1 class="mt-3 truncate text-2xl font-black sm:text-3xl">
                {{ teacherName }}
              </h1>
          ...
          <div class="grid grid-cols-2 gap-2 sm:grid-cols-4 xl:min-w-[520px]">
            <HeroStat
              ...
              :value="
                formatPoints(ranking?.educational_qualification_total_points)
              "
            />
            <HeroStat
              label="Evaluation"
            ...
            <HeroStat
              label="Salary Rate"
              :value="formatPlainAmount(getAssignedSalaryRate(ranking))"
            />
          </div>
        </div>
      </section>
      ...
      <section
        v-if="!ranking"
        class="rounded-[28px] border border-amber-200 bg-amber-50 p-10 text-center dark:border-amber-900 dark:bg-amber-950/20"
      >
        <div
          class="mx-auto flex size-14 items-center justify-center rounded-2xl bg-amber-100 text-amber-600 dark:bg-amber-950/40"
        >
          <UIcon name="i-lucide-calculator" class="size-7" />
        </div>
        ...
        <p class="mt-2 text-sm text-gray-500">
          Compute this personnel ranking to view the complete point breakdown,
          assigned rank, and salary rate.
        </p>
      ...
      <template v-else>
        ...
        >
          ...
          >
            <div>
              ...
              <h2 class="mt-1 text-lg font-black text-gray-900 dark:text-white">
                Major Ranking Criteria
              </h2>

              <p class="mt-1 text-xs text-gray-500">
                The four criteria used to determine the faculty's final ranking.
              </p>
            </div>

            <div
              class="rounded-2xl bg-gray-50 px-4 py-3 text-left sm:text-right dark:bg-gray-950/40"
            >
              ...
              <p
                class="mt-1 text-2xl font-black text-blue-700 dark:text-blue-400"
              >
                {{ formatPoints(ranking.total_ranking_points) }}
                <span class="text-xs font-semibold text-gray-500">pts</span>
              </p>
            </div>
          </div>
          ...
          <div class="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            <div
              v-for="item in majorCriteria"
              :key="item.key"
              class="rounded-2xl border border-gray-200 bg-gray-50/70 p-4 transition hover:border-blue-200 hover:bg-blue-50/40 dark:border-gray-800 dark:bg-gray-950/30 dark:hover:border-blue-900 dark:hover:bg-blue-950/10"
            >
              <div class="flex items-start justify-between gap-3">
                <div
                  class="flex size-10 shrink-0 items-center justify-center rounded-xl bg-white text-blue-600 shadow-sm ring-1 ring-gray-100 dark:bg-gray-900 dark:text-blue-400 dark:ring-gray-800"
                >
                  <UIcon :name="item.icon" class="size-5" />
                </div>

                <span
                  class="rounded-xl bg-white px-3 py-1.5 text-base font-black text-gray-900 shadow-sm ring-1 ring-gray-100 dark:bg-gray-900 dark:text-white dark:ring-gray-800"
                >
        ...
        <section class="grid gap-6 xl:grid-cols-[1.35fr_0.65fr]">
          ...
          <div class="space-y-6">
            ...
            >
              <div
                class="flex flex-col gap-4 border-b border-gray-200 px-5 py-5 sm:flex-row sm:items-center sm:justify-between dark:border-gray-800"
              >
                <div class="flex items-center gap-3">
                  <div
                    class="flex size-11 shrink-0 items-center justify-center rounded-2xl bg-blue-50 text-blue-600 dark:bg-blue-950/30 dark:text-blue-400"
                  >
                  ...
                  <div>
                    ...
                    <p class="mt-1 text-xs text-gray-500">
                      Detailed academic and professional qualification points.
                    </p>
                  </div>
                </div>

                <div
                  class="rounded-2xl bg-blue-50 px-4 py-2.5 text-left sm:text-right dark:bg-blue-950/20"
                >
                  <p
                  ...
                  <p
                    class="mt-0.5 text-xl font-black text-blue-700 dark:text-blue-300"
                  >
                    {{
                      formatPoints(
                        ranking.educational_qualification_total_points,
                      )
                    }}
                  </p>
                </div>
              ...
              <div class="grid sm:grid-cols-2">
                <div
                  v-for="(item, index) in educationalQualificationRows"
                  :key="item.key"
                  class="flex items-center justify-between gap-4 border-gray-100 p-5 dark:border-gray-800"
                  :class="[
                    index < educationalQualificationRows.length - 2
                      ? 'border-b'
                      : '',
                ...
                >
                  ...
                  <span
                    class="shrink-0 text-base font-black text-gray-900 dark:text-white"
                  >
                    {{ formatPoints(item.points) }}
                  </span>
                </div>
            ...
            >
              ...
              >
                <div class="flex items-center gap-3">
                  <div
                    class="flex size-11 shrink-0 items-center justify-center rounded-2xl bg-emerald-50 text-emerald-600 dark:bg-emerald-950/30 dark:text-emerald-400"
                  >
                  ...
                  <div>
                    ...
                    <p class="mt-1 text-xs text-gray-500">
                      Automatic and HR-maintained evaluation components.
                    </p>
                  </div>
                </div>

                <UBadge color="success" variant="subtle" size="lg">
                  {{ formatPoints(ranking.evaluation_points) }} total points
                </UBadge>
              </div>
          ...
          <div class="space-y-6">
            ...
            >
              <div
                class="bg-gradient-to-br from-blue-600 via-blue-700 to-cyan-700 px-5 py-6 text-white"
              >
                <p
                  class="text-[10px] font-bold uppercase tracking-[0.16em] text-blue-100"
                >
                ...
                <div class="mt-4 flex items-center gap-3">
                  <div
                    class="flex size-12 shrink-0 items-center justify-center rounded-2xl bg-white/15 ring-1 ring-white/20"
                  >
                  ...
                  <div class="min-w-0">
                    <h2 class="truncate text-2xl font-black">
                      {{ ranking.rank_name || "Not assigned" }}
                    </h2>

                    <p class="mt-1 text-xs text-blue-100">
                      {{ ranking.rank_code || "No rank code" }}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div class="space-y-4 px-5 py-5">
                ...
                <InfoRow
                  label="Salary Rate"
                  :value="formatCurrency(getAssignedSalaryRate(ranking))"
                />
              ...
              <div
                ...
                class="border-t border-gray-200 bg-gray-50 px-5 py-4 text-xs leading-5 text-gray-600 dark:border-gray-800 dark:bg-gray-950/40 dark:text-gray-300"
              >
                {{ ranking.rate_assignment_message }}
              </div>
            </section>

            <!-- ACADEMIC / SCHEME INFO -->
            <section
              class="rounded-[26px] border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900"
            >
              <div class="flex items-center gap-3">
                ...
                >
                  <UIcon name="i-lucide-file-chart-column" class="size-5" />
                </div>
                ...
                <div>
                  ...
                  <p class="mt-1 text-xs text-gray-500">
                    Scheme and academic period used for this computation.
                  </p>
                </div>
              </div>
              ...
              <div class="mt-5 space-y-4">
                ...
                <InfoRow
                  label="School Year"
                  :value="ranking.school_year || 'N/A'"
                />

                <InfoRow label="Semester" :value="ranking.semester || 'N/A'" />
              </div>
            </section>
            ...
            >
              <div class="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p class="mt-1 text-xs text-gray-500">
                    Other major criteria contributing to the final score.
                  </p>
                </div>
              </div>
              ...
              <div class="mt-5 space-y-4">
                <InfoRow
                  label="Loyalty"
                  :value="`${formatPoints(ranking.loyalty_points)} points`"
                />

                <InfoRow
                  label="Corporate Social Responsibility"
                  :value="`${formatPoints(ranking.csr_points)} points`"
                />
              </div>
            </section>
          </div>
    ...
    <UModal
      ...
      id="faculty-ranking-pdf-preview-dialog"
      title="Faculty Ranking PDF Preview"
      description="Preview the official faculty ranking report before downloading the PDF."
          ...
          >
            <div>
              ...
              <p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">
                Official Faculty Ranking Report
              </p>
            </div>
      ...
      <template #content>
        ...
        >
          ...
          >
            ...
            >
              Recomputing will refresh portfolio points, evaluation points,
              assigned rank, and salary rate using the selected ranking scheme.
            </div>
...
const reportDocumentCode = "SNC-HR-FR";
...
const reportNumber = computed(() => {
  const source =
    ...
    ranking.value?.documentId ||
    teacher.value?.employee_no ||
    teacher.value?.documentId ||
    "faculty-ranking";
...
const pdfFileName = computed(() => {
  const cleanName = String(teacherName.value || "Faculty")
    .trim()
    .replace(/[^a-zA-Z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "");

  const schoolYear = String(ranking.value?.school_year || "Academic-Year")
    .trim()
    .replace(/[^a-zA-Z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "");

  return `Faculty-Ranking-${cleanName}-${schoolYear}.pdf`;
});
...
const educationalQualificationRows = computed(() => [
  {
...
const majorCriteria = computed(() => [
  {
    key: "educational_qualification",
    label: "Educational Qualification",
    points: ranking.value?.educational_qualification_total_points,
    icon: "i-lucide-graduation-cap",
    description:
      "Academic attainment, eligibility, training, research, awards, and experience.",
  },
  {
    key: "loyalty",
    label: "Loyalty",
    ...
    description: "Service and institutional loyalty points.",
  },
  {
    key: "evaluation",
    label: "Evaluation",
    points: ranking.value?.evaluation_points,
    icon: "i-lucide-clipboard-check",
    description: "Student, immediate superior, and HR evaluation points.",
  },
  {
    key: "csr",
    label: "Corporate Social Responsibility",
    ...
    description: "Community and institutional social responsibility points.",
  },
]);
      ...
      h(
        ...
        [
          ...
          h(
            "p",
            {
              class: "mt-1 text-[10px] uppercase tracking-wide text-blue-100",
            },
            props.label,
          ),
        ],
  ...
  setup(props) {
    return () =>
      h(
        ...
        {
          class:
            "rounded-2xl border border-gray-200 bg-gray-50/60 p-4 dark:border-gray-800 dark:bg-gray-950/30",
        },
        [
...
const openComputeDialog = () => {
  ...
  computeForm.ranking_scheme_document_id = scheme?.documentId || "";

  computeForm.school_year =
    ranking.value?.school_year || scheme?.academic_year || "2026-2027";

  computeForm.semester = ranking.value?.semester || "1st Semester";
...
const computeRanking = async () => {
  ...
  try {
    await $api(`/faculty-rankings/compute/${teacherDocumentId}`, {
      ...
      body: {
        data: {
          ranking_scheme_document_id: computeForm.ranking_scheme_document_id,
...
const loadData = async () => {
  ...
  if (!teacherDocumentId || ["undefined", "null"].includes(teacherDocumentId)) {
    loadError.value =
  ...
  try {
    ...
    if (requestedRankingDocumentId) {
      ...
      ranking.value = rankingResponse?.data || rankingResponse || null;
    } else {
      ...
      ranking.value = rankingResponse?.data?.[0] || null;
    }
  } catch (error: any) {
    teacher.value = null;
    ranking.value = null;
...
const getAssignedSalaryRate = (rankingRecord: any): number | null => {
  const storedRate = Number(rankingRecord?.salary_rate);

  if (Number.isFinite(storedRate) && storedRate > 0) {
    return storedRate;
  }

  const relatedRate = Number(rankingRecord?.salary_rate_record?.max_rate);

  if (Number.isFinite(relatedRate) && relatedRate > 0) {
    return relatedRate;
  }
...
const createPdfBlob = async (): Promise<Blob> => {
  ...
  if (!teacher.value || !ranking.value) {
    throw new Error("No faculty ranking result is available.");
  }
  ...
  if (SNC_LOGO_DATA_URL) {
    try {
      doc.addImage(SNC_LOGO_DATA_URL, "JPEG", marginLeft, 11, 19, 19);
    } catch {
      // Continue without logo if image rendering fails.
    }
  }
  ...
  doc.text("ST. NICOLAS COLLEGE OF BUSINESS AND TECHNOLOGY", headerCenter, 15, {
    align: "center",
  });
  ...
  doc.text("Human Resources Office", headerCenter, 20, {
    align: "center",
  });
  ...
  doc.text("FACULTY RANKING REPORT", headerCenter, 27, {
    align: "center",
  });
  ...
  addSectionTitle("I. FACULTY INFORMATION");
  ...
  addTable(
    ...
    [
      [
        ...
        teacherName.value,
        "Employee No.",
        teacher.value?.employee_no || teacher.value?.user?.username || "N/A",
      ],
      ["Department", departmentName.value, "Role", teacherRole.value],
      [
        "School Year",
        ranking.value?.school_year || "N/A",
        "Semester",
        ranking.value?.semester || "N/A",
      ],
      [
        "Ranking Scheme",
        ranking.value?.ranking_scheme?.name || "Not specified",
        "Computed At",
        formatDateTime(ranking.value?.computed_at),
      ],
    ],
    {
      columnStyles: {
  ...
  // II. RANKING SUMMARY
  addSectionTitle("II. RANKING SUMMARY");

  addTable(
    [["Major Criterion", "Points"]],
    [
      [
        "Educational Qualification",
        formatPoints(ranking.value?.educational_qualification_total_points),
      ],
      ["Loyalty", formatPoints(ranking.value?.loyalty_points)],
      ["Evaluation", formatPoints(ranking.value?.evaluation_points)],
      [
        "Corporate Social Responsibility",
        formatPoints(ranking.value?.csr_points),
      ],
      ["TOTAL POINTS", formatPoints(ranking.value?.total_ranking_points)],
    ],
    {
      columnStyles: {
        0: {
  ...
  addSectionTitle("III. EDUCATIONAL QUALIFICATION BREAKDOWN");

  addTable(
    [["Component", "Points"]],
    educationalQualificationRows.value.map((item: any) => [
      item.label,
      formatPoints(item.points),
    ]),
    {
  ...
  // IV. EVALUATION
  addSectionTitle("IV. EVALUATION BREAKDOWN");

  addTable(
    [["Evaluation Source", "Points", "Maximum", "Rating", "Source"]],
    [
      [
        "Student Evaluation",
        formatPoints(ranking.value?.student_evaluation_points),
        formatPoints(studentMaximum.value),
        studentRating.value,
        "Automatic",
      ],
      [
        "Immediate Superior Evaluation",
        formatPoints(ranking.value?.immediate_superior_evaluation_points),
        formatPoints(superiorMaximum.value),
        superiorRating.value,
        isDean.value ? "Manual HR Entry" : "Automatic",
      ],
      [
        "HR Evaluation",
        formatPoints(ranking.value?.hr_evaluation_points),
        formatPoints(hrMaximum.value),
        hrRating.value,
        "Manual HR Entry",
      ],
    ],
    {
      ...
      foot: [
        [
          "Total Evaluation",
          formatPoints(ranking.value?.evaluation_points),
          "",
          "",
          "",
        ],
  ...
  // V. RANK AND SALARY RATE
  addSectionTitle("V. RANK AND SALARY RATE ASSIGNMENT");

  addTable(
    [["Item", "Assigned Value"]],
    [
      ["Assigned Rank", ranking.value?.rank_name || "Not assigned"],
      ["Rank Code", ranking.value?.rank_code || "N/A"],
      [
        "Total Ranking Points",
        formatPoints(ranking.value?.total_ranking_points),
      ],
      [
        "Salary Rate",
        `${formatPlainAmount(getAssignedSalaryRate(ranking.value))}`,
      ],
      [
        "Rate Assignment Status",
        formatStatus(ranking.value?.rate_assignment_status),
      ],
      ["Computation Status", formatStatus(ranking.value?.computation_status)],
    ],
    {
      columnStyles: {
        0: {
  ...
  // VI. REMARKS
  if (ranking.value?.rate_assignment_message || ranking.value?.remarks) {
    addSectionTitle("VI. COMPUTATION NOTES");

    const notes: any[][] = [];

    if (ranking.value?.rate_assignment_message) {
      notes.push(["Rate Assignment", ranking.value.rate_assignment_message]);
    }

    if (ranking.value?.remarks) {
      notes.push(["Remarks", ranking.value.remarks]);
    }

    addTable([["Item", "Details"]], notes, {
      columnStyles: {
        0: {
          cellWidth: 42,
          fontStyle: "bold",
  ...
  // VII. CERTIFICATION
  addSectionTitle("VII. CERTIFICATION AND SIGN-OFF");
  ...
  const certificationText =
    "This report was generated from the official Faculty Ranking System and reflects the portfolio, evaluation, rank-band, and salary-rate records available for the stated academic period. It is intended for authorised institutional human resource, academic, and administrative purposes.";
  ...
  doc.text(certificationLines, marginLeft, y);

  y += certificationLines.length * 4 + 8;
  ...
  const signatures = [
    ...
    ["Approved / Noted by", "School Administrator"],
  ];
  ...
  signatures.forEach(([label, role], index) => {
    ...
    doc.text(label, x + signatureWidth / 2, y + 20, {
      align: "center",
    });
    ...
    doc.text(role, x + signatureWidth / 2, y + 24, {
      align: "center",
    });
...
const previewPdfReport = async () => {
  ...
  try {
    ...
    console.error("Faculty ranking PDF preview generation error:", error);

    toast.add({
      title: "Unable to preview PDF",
      description:
        error?.message || "The faculty ranking PDF could not be generated.",
...
const downloadPdfReport = async () => {
  ...
  try {
    ...
    console.error("Faculty ranking PDF download generation error:", error);
