      ...
      <UDashboardNavbar>
        ...
        <template #title>
          <span class="text-sm text-gray-500 uppercase">
            Faculty to Dean Evaluation Results
          </span>
        </template>
      </UDashboardNavbar>
    </template>
      ...
      <div class="space-y-6">
        ...
        <div class="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
          <div class="grid grid-cols-1 gap-4 md:grid-cols-5">
            <UFormField label="Dean / Coordinator">
              <USelectMenu
                v-model="filters.dean_id"
                :items="deanOptions"
                value-key="value"
                placeholder="Select dean / coordinator"
                class="w-full"
              />
            </UFormField>
        ...
        <div class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
          ...
          <div v-else-if="!evaluations.length" class="py-12 text-center text-gray-500">
            No Faculty to Dean evaluation results found.
          </div>
          ...
          <div v-else class="overflow-x-auto">
            <table class="w-full text-sm">
              <thead class="bg-gray-50 text-xs uppercase text-gray-600">
                <tr>
                  <th class="px-4 py-3 text-left">Dean / Coordinator</th>
              ...
              <tbody class="divide-y divide-gray-100">
                ...
                >
                  <td class="px-4 py-3 font-medium text-gray-800">
                    {{ evaluation.dean_coordinator?.name || 'Unknown Dean' }}
                  </td>

                  <td class="px-4 py-3 text-gray-600">
                    {{ evaluation.dean_coordinator?.department || evaluation.batch?.department || 'N/A' }}
                  </td>
            ...
            >
              <div class="mb-5">
                <h2 class="text-xl font-bold text-gray-800">
                  {{ selectedEvaluation.dean_coordinator?.name || 'Unknown Dean' }}
                </h2>
                <p class="text-sm text-gray-500">
                  {{ selectedEvaluation.dean_coordinator?.department || selectedEvaluation.batch?.department || 'N/A' }}
                </p>
              </div>
...
const getResults = async () => {
  ...
  try {
    ...
    if (filters.dean_id) {
      query['filters[dean_coordinator][id][$eq]'] = filters.dean_id
    }
