  ...
  <div class="space-y-6 pb-10">
        ...
        <div class="flex min-w-0 items-start gap-4">
          ...
          <div class="min-w-0">
            ...
            <p class="mt-2 max-w-3xl text-sm leading-6 text-emerald-50/90">
              Register, update, and manage faculty accounts within your
              department.
            </p>
          </div>
        </div>
      ...
      >
        ...
        <div class="min-w-0 flex-1">
          <div class="flex flex-wrap items-center gap-2">
            ...
            <UBadge color="success" variant="subtle" size="sm"> Dean </UBadge>
          </div>
    ...
    >
      <UIcon
        ...
        class="mx-auto size-7 text-red-600 dark:text-red-400"
      />
      ...
      >
        <div class="overflow-x-auto">
          <table class="min-w-[900px] w-full border-collapse">
            <thead>
              <tr
                class="border-b border-gray-200 bg-gray-50 dark:border-gray-800 dark:bg-gray-950/40"
              >
                ...
                <th
                  class="px-4 py-3 text-left text-[10px] font-semibold uppercase tracking-wide text-gray-500"
                >
                  Employee No.
                </th>
                ...
                <th
                  class="w-20 px-4 py-3 text-center text-[10px] font-semibold uppercase tracking-wide text-gray-500"
                >
                  Action
                </th>
              </tr>
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-800">
              <tr v-if="loading">
                <td colspan="6" class="px-4 py-12 text-center">
                  <UIcon
                    ...
                    class="mx-auto size-6 animate-spin text-emerald-500"
                  />
              ...
              <tr v-else-if="paginatedTeachers.length === 0">
                <td colspan="6" class="px-4 py-12 text-center">
                  ...
                  <p
                    class="mt-3 text-sm font-semibold text-gray-700 dark:text-gray-300"
                  >
                    No faculty found
                  </p>
                ...
                <td class="px-4 py-3">
                  <div class="flex min-w-0 items-center gap-3">
                    ...
                    <div class="min-w-0">
                      ...
                      <p class="mt-0.5 text-[11px] text-gray-400">Faculty</p>
                    </div>
                  </div>
                </td>
        ...
        >
          <div class="text-xs text-gray-500 dark:text-gray-400">
            {{ selectedCount }} of {{ filteredTeachers.length }} row(s)
            selected.
          </div>
    ...
    <UModal v-model:open="createModal" :ui="{ content: 'max-w-3xl' }">
      <template #content>
        <div
          class="max-h-[90vh] overflow-y-auto rounded-[28px] border border-gray-200 bg-white shadow-2xl dark:border-gray-800 dark:bg-gray-900"
        >
          <div
          ...
          >
            ...
            <div class="relative flex items-start justify-between gap-4">
              <div class="flex items-center gap-4">
                <div
                  class="flex size-12 items-center justify-center rounded-2xl border border-white/15 bg-white/10 text-emerald-300"
                >
                ...
                <div>
                  ...
                  <h2 class="mt-1 text-xl font-bold">Register Faculty</h2>
            ...
            <div class="relative flex items-start justify-between gap-4">
              <div class="flex items-center gap-4">
                ...
                <div>
                  ...
                  <h2 class="mt-1 text-xl font-bold">Edit Faculty</h2>
    ...
    <UModal v-model:open="deleteModal">
      <template #content>
        <div
          class="rounded-[28px] border border-gray-200 bg-white p-6 shadow-2xl dark:border-gray-800 dark:bg-gray-900"
        >
          <div
            class="mx-auto flex size-16 items-center justify-center rounded-2xl bg-red-100 text-red-600 dark:bg-red-950 dark:text-red-400"
          >
...
const deanDepartmentId = computed(() => {
  return deanProfile.value?.department?.id || null;
});
...
const allVisibleSelected = computed(() => {
  ...
  return paginatedTeachers.value.every((teacher) => isRowSelected(teacher));
});
...
const toggleSelectVisible = (value: boolean) => {
  paginatedTeachers.value.forEach((teacher) => {
    const key = teacher.documentId || teacher.id;
...
const isRowSelected = (row: any) => {
  ...
  return Boolean(selectedRows.value[key]);
};

const openSingleDeleteConfirmation = (row: any) => {
...
const getDeanProfile = async () => {
  ...
  deanProfile.value = response.data?.[0] || null;
};
...
const getTeachers = async () => {
  ...
  const response = await $api("/teachers", {
    query: {
      ...
      "filters[department][id][$eq]": deanDepartmentId.value,
  ...
  try {
    ...
    await $api("/teachers/register", {
      ...
      body: {
        ...
        department: deanDepartmentId.value,

        roleName: "Faculty",

        username: createForm.employee_no.trim(),
...
const updateTeacher = async () => {
  ...
  try {
    ...
    await $api(`/teachers/update-with-user/${selectedId.value}`, {
      method: "PUT",
      body: {
        employee_no: editForm.employee_no.trim(),
    ...
    toast.add({
      ...
      description: "Faculty information was updated successfully.",
      icon: "i-lucide-circle-check-big",
      color: "success",
    });
...
const deleteOne = async (row: any) => {
  await $api(`/teachers/delete-with-user/${row.id}`, {
    method: "DELETE",
  });
...
const deleteSelected = async () => {
  ...
  await Promise.all(
    selectedItems.map((item: any) =>
      $api(`/teachers/delete-with-user/${item.id}`, {
        method: "DELETE",
      }),
    ),
  );
  ...
  try {
    ...
    if (deleteMode.value === "bulk") {
      ...
      toast.add({
        ...
        description: "The selected faculty accounts were deleted successfully.",
        icon: "i-lucide-circle-check-big",
        color: "success",
      });
      ...
      toast.add({
        title: "Faculty deleted",
        description: "The faculty account was deleted successfully.",
        icon: "i-lucide-circle-check-big",
        color: "success",
      });
    }
...
const loadData = async () => {
  try {
    ...
    if (!deanDepartmentId.value) {
      throw new Error("The logged-in Dean is not assigned to a department.");
    }

    await getTeachers();
  } catch (error: any) {
    console.error("Department faculty management loading error:", error);
