        ...
        <div class="flex min-w-0 items-start gap-4">
          ...
          <div class="min-w-0">
            <div class="mb-2 flex flex-wrap items-center gap-2 text-xs font-medium text-emerald-50">
              <span class="rounded-full border border-white/20 bg-white/10 px-2.5 py-1 backdrop-blur">Dean Portal</span>
              <span class="rounded-full border border-white/20 bg-white/10 px-2.5 py-1 backdrop-blur">Dean – Faculty</span>
            </div>

            <h1 class="text-2xl font-bold tracking-tight sm:text-3xl">Faculty Evaluation</h1>
            <p class="mt-2 max-w-3xl text-sm leading-6 text-emerald-50/90">
              Evaluate assigned faculty members in your department by rating all criteria and providing constructive written feedback.
            </p>
          </div>
        </div>
    ...
    <section class="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
      <div class="flex items-center gap-4 px-5 py-4 sm:px-6">
        ...
        <div class="hidden rounded-xl border border-emerald-100 bg-emerald-50 px-3 py-2 text-right sm:block dark:border-emerald-900 dark:bg-emerald-950/30">
          ...
          <p class="mt-0.5 text-xs font-bold text-gray-800 dark:text-gray-200">Dean – Faculty</p>
        </div>
      </div>
    </section>
      ...
      <div v-else class="grid grid-cols-1 items-start gap-6 xl:grid-cols-[300px_minmax(0,1fr)]">
        <aside class="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm xl:sticky xl:top-5 dark:border-gray-800 dark:bg-gray-900">
          <div class="border-b border-gray-200 bg-gradient-to-br from-gray-50 to-white px-5 py-4 dark:border-gray-800 dark:from-gray-900 dark:to-gray-900">
            <div class="flex items-start justify-between gap-3">
              <div>
                ...
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Select one or more faculty members to evaluate.</p>
              </div>
          ...
          <div class="max-h-[430px] overflow-y-auto p-3">
            <div v-if="targetOptions.length === 0" class="px-3 py-10 text-center">
              ...
              <p class="mt-3 text-sm font-medium text-gray-700 dark:text-gray-300">No assigned faculty found</p>
        ...
        <div class="min-w-0 space-y-5">
          <section v-if="!selectedTargetIds.length" class="rounded-2xl border border-dashed border-gray-300 bg-white px-6 py-14 text-center dark:border-gray-700 dark:bg-gray-900">
            ...
            <h3 class="mt-5 text-lg font-bold text-gray-900 dark:text-white">Select a faculty member</h3>
            <p class="mx-auto mt-2 max-w-md text-sm leading-6 text-gray-500 dark:text-gray-400">Choose at least one assigned faculty member from the left panel.</p>
          </section>
          ...
          <template v-else>
            ...
            <section v-for="evaluation in paginatedEvaluations" :key="evaluation.teacherId" class="rounded-[24px] border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
              <div class="relative overflow-hidden rounded-t-[24px] border-b border-gray-200 bg-gradient-to-br from-slate-900 via-slate-800 to-emerald-950 px-5 py-6 text-white sm:px-7 dark:border-gray-800">
                ...
                <div class="relative flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
                  <div class="flex min-w-0 items-center gap-4">
                    ...
                    <div class="min-w-0">
                      <p class="text-xs font-semibold uppercase tracking-[0.16em] text-emerald-300">Dean-Faculty Evaluation</p>
                      <h2 class="mt-1 truncate text-xl font-bold">{{ targetMap[String(evaluation.teacherId)]?.name || 'Unknown' }}</h2>
              ...
              <div class="space-y-7 p-4 sm:p-6 lg:p-7">
                  ...
                  <div class="flex items-start gap-3">
                    ...
                    <div class="min-w-0 flex-1">
                      ...
                      <p class="mt-1 text-xs leading-5 text-gray-600 dark:text-gray-400">
                        Select one rating for every criterion. This scale is loaded
                        from the <strong>{{ evaluationType?.name }}</strong>
                ...
                <div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
                  <div class="overflow-hidden rounded-2xl border border-emerald-100 dark:border-emerald-900">
                    <div class="border-b border-emerald-100 bg-emerald-50 px-4 py-3.5 dark:border-emerald-900 dark:bg-emerald-950/20">
                      <h3 class="flex items-center gap-2 text-sm font-bold text-gray-900 dark:text-white">
                        <UIcon name="i-lucide-thumbs-up" class="size-4 text-emerald-500" />
                        Greatest Strengths
                      </h3>
                    </div>
                    <div class="p-4">
                      <UTextarea
                        ...
                        :rows="5"
                        autoresize
                        placeholder="Describe the teacher's strongest qualities..."
                        class="w-full"
                      />
                    </div>
                  </div>

                  <div class="overflow-hidden rounded-2xl border border-blue-100 dark:border-blue-900">
                    <div class="border-b border-blue-100 bg-blue-50 px-4 py-3.5 dark:border-blue-900 dark:bg-blue-950/20">
                      <h3 class="flex items-center gap-2 text-sm font-bold text-gray-900 dark:text-white">
                    ...
                    <div class="p-4">
                      <UTextarea
                        v-model="evaluation.effectiveness"
                        :rows="5"
                        autoresize
                        placeholder="Describe how the teacher could be more effective..."
                        class="w-full"
                      />
                    </div>
                  </div>
                  ...
                  <div class="overflow-hidden rounded-2xl border border-amber-100 dark:border-amber-900">
                    ...
                    <div class="p-4">
                      <UTextarea
                        v-model="evaluation.areas_for_improvement"
                        :rows="5"
                        autoresize
                        placeholder="Suggest areas that may be improved..."
                        class="w-full"
                      />
                    </div>
                  </div>

                  <div class="overflow-hidden rounded-2xl border border-cyan-100 dark:border-cyan-900">
                    <div class="border-b border-cyan-100 bg-cyan-50 px-4 py-3.5 dark:border-cyan-900 dark:bg-cyan-950/20">
                      <h3 class="flex items-center gap-2 text-sm font-bold text-gray-900 dark:text-white">
                        <UIcon name="i-lucide-graduation-cap" class="size-4 text-cyan-500" />
                        Suggested Improvement Activities
                      </h3>
                    </div>
                    <div class="p-4">
                      <UTextarea
                        v-model="evaluation.suggested_activities"
                        :rows="5"
                        autoresize
                        placeholder="Suggest seminars, training, coaching, or other activities..."
                        class="w-full"
                      />
                    </div>
                  </div>
                ...
                <div class="overflow-hidden rounded-2xl border border-violet-100 dark:border-violet-900">
                  ...
                  <div class="p-4">
                    <UTextarea
                      ...
                      :rows="4"
                      autoresize
                      placeholder="Explain your overall rating and add constructive comments..."
                      class="w-full"
                    />
                  </div>
                </div>
        ...
        <div class="relative overflow-hidden rounded-[28px] border border-white/20 bg-white/95 shadow-[0_20px_80px_rgba(15,23,42,0.20)] backdrop-blur-2xl dark:border-gray-800 dark:bg-gray-900/95">
          ...
          <div class="relative p-7">
            ...
            <div class="mt-5 text-center">
              ...
              <p class="mt-2 text-sm leading-6 text-gray-500 dark:text-gray-400">You are submitting <strong>{{ form.evaluations.length }}</strong> Faculty evaluation(s). Submitted evaluations may no longer be edited.</p>
            </div>
...
const targetOptions = computed(() =>
  targets.value.map((item: any) => {
    const alreadyEvaluated = existingEvaluations.value.some(
      (evaluation: any) => Number(evaluation.teacher?.id) === Number(item.id)
    )

    return {
      label: alreadyEvaluated ? `${item.name} (Already Evaluated)` : item.name,
      description: alreadyEvaluated
        ? 'Faculty evaluation already submitted for this period.'
...
const currentEvaluationTargetName = computed(() => {
  ...
  if (!evaluation) return 'No faculty selected'
  return targetMap.value[String(evaluation.teacherId)]?.name || 'Unknown'
})
...
const getLoggedInDean = async () => {
  ...
  loggedInDean.value = response.data?.[0] || null
...
const getTargets = async () => {
  const deanProfile = loggedInDean.value || await getLoggedInDean()
...
const getExistingEvaluations = async () => {
  ...
  try {
    ...
    /*
     * IMPORTANT:
     * Do not filter previous evaluations by evaluation_type.code here.
     *
     * The purpose of this query is duplicate prevention for this Dean during
     * the active semester and school year. Older evaluation batches may still
     * reference a previous Evaluation Type record/code even though the current
     * configured Evaluation Type is now "dean-faculty".
     *
     * Filtering only by evaluator + period lets us correctly restore the
     * disabled faculty list after a page reload.
     */
    ...
    /*
     * Keep only evaluation records that actually target Faculty members shown
     * on this page. This avoids unrelated evaluation records from disabling
     * anything accidentally.
     */
    const targetIds = new Set(
      targets.value.map((target: any) => Number(target.id))
    )

    existingEvaluations.value = (response.data || []).filter(
      (evaluation: any) =>
        evaluation.teacher?.id &&
        targetIds.has(Number(evaluation.teacher.id))
    )
  } catch (error) {
...
const loadData = async () => {
  try {
    ...
    if (!evaluationType.value) {
      throw new Error('Dean-Faculty evaluation type is not configured.')
    }
    ...
    await getLoggedInDean()

    if (!loggedInDean.value) {
      throw new Error('The logged-in Dean profile could not be found.')
    }

    if (!form.department) {
      throw new Error('The Dean is not assigned to a department.')
    }

    await Promise.all([getTargets(), getSections()])
    await getExistingEvaluations()
  } catch (error: any) {
    console.error('Dean evaluation loading error:', error)
    loadError.value =
...
const openSubmitConfirmation = async () => {
  ...
  if (!isFormValid.value) {
    ...
    toast.add({
      ...
      description: 'Complete every selected Faculty evaluation before submitting.',
...
const submitEvaluation = async () => {
  ...
  try {
    ...
    toast.add({
      ...
      description: 'Dean-Faculty evaluations were submitted successfully.',
    ...
    console.error('Dean evaluation submission error:', error)
    confirmationOpen.value = false
    submitError.value =
      error?.data?.error?.message ||
      error?.data?.message ||
